...
st.set_page_config(page_title='RFT Automatico - V9.0.1', page_icon='R', layout='wide', initial_sidebar_state='expanded')
...
def save_raw(conn, upload_id, df):
    ...
    for _, row in df.iterrows():
        rows.append((
            int(upload_id),
            row['NR_WO'],
            row['DT_HR_INSPECAO'].isoformat(sep=' ', timespec='seconds'),
            float(row['C_DPU_QG_AMARELO']),
            row['CD_POSTO_CN'],
        ))
    conn.executemany(
...
def load_merged_year_df(conn, posto, year):
    df = pd.read_sql_query(
        "SELECT upload_id, nr_wo AS NR_WO, dt_hr_inspecao AS DT_HR_INSPECAO, c_dpu_qg_amarelo AS C_DPU_QG_AMARELO, cd_posto_cn AS CD_POSTO_CN FROM raw_inspections WHERE cd_posto_cn=? AND strftime('%Y', dt_hr_inspecao)=? ORDER BY upload_id ASC, id ASC",
        conn,
        params=[posto, str(year)],
    )
    if df.empty:
...
def resolve_period_selection(df, ano, mode):
    ...
    if mode == 'Diario':
        ...
        if default_day < min_date or default_day > max_date:
            default_day = max_date
        selected = st.sidebar.date_input('Dia', value=default_day, min_value=min_date, max_value=max_date, format='DD/MM/YYYY', key='day_v901')
        ...
        label = st.sidebar.selectbox('Semana do ano', labels, index=idx, key='week_v901')
        ...
        idx = labels.index(saved) if saved in labels else len(labels) - 1
        label = st.sidebar.selectbox('Mes do ano', labels, index=idx, key='month_v901')
...
def main():
    conn = get_conn()
    init_db(conn)
    st.title('RFT Automatico - V9.0.1')
    st.caption('Correção da V9.0.1: upload funcionando corretamente, com calendário, YTD e tendência mensal em barras.')
    ...
    with tabs[0]:
        if ano is None or df.empty:
            ...
            _, max_date, selected_label, selected_range, valid = resolve_period_selection(df, ano, mode)
            if not valid:
                ...
                items = [
                    ...
                    ('RFT Anual', 'Ano ate 12/12', 'yearly'),
                    ('RFT YTD', 'Acumulado ate o recorte', 'ytd'),
                ]
                for col, (title, sub, key) in zip(cols, items):
                    with col:
                        ...
                        if res is None:
                            ...
                            st.metric(title, format_pct(res['rft_pct']), f"{sub} | WOs boas: {res['good']} | ruins: {res['bad']} | total: {res['total']}")
        ...
        if ano is None or df.empty:
            ...
            with c1:
                ...
                if monthly_df.empty:
                    ...
                    monthly_show['RFT'] = monthly_show['RFT'].map(lambda x: f'{x:.2f}'.replace('.', ',') + '%')
                    monthly_show['Meta'] = monthly_show['Meta'].map(lambda x: f'{x:.2f}'.replace('.', ',') + '%')
            ...
            with c2:
                ...
                if weekly_df.empty:
                    ...
                    weekly_show['RFT'] = weekly_show['RFT'].map(lambda x: f'{x:.2f}'.replace('.', ',') + '%')
                    st.dataframe(weekly_show, use_container_width=True, hide_index=True)

    with tabs[2]:
        st.subheader('Base & Upload')
        st.caption('Correção da V9.0.1: o seletor de arquivo agora usa chave dedicada e fluxo estável para permitir anexar normalmente.')
        import_mode = st.radio('Modo de importação', ['Somar ao historico', 'Substituir periodo sobreposto', 'Reprocessar o ano inteiro'], key='import_mode_v901')
        uploaded = st.file_uploader('Base operacional atual (.xlsx, .xls ou .csv)', type=['xlsx', 'xls', 'csv'], key='file_uploader_v901')
        prepared = None
        if uploaded is not None:
            try:
                ...
                if not ok:
                    ...
                    st.success(f'Arquivo carregado: {uploaded.name} | Linhas válidas após tratamento: {len(prepared)}')
                    if not impact.empty:
        ...
        if st.button('Salvar arquivo localmente', type='primary', use_container_width=True, key='save_upload_v901'):
            if uploaded is None:
    ...
    with tabs[3]:
        ...
        if hist.empty:
            ...
            selected_id = st.selectbox('Selecionar upload', hist['id'].tolist(), format_func=lambda x: f'Upload {x}', key='history_select_v901')
            c1, c2, c3 = st.columns(3)
            with c1:
                if st.button('Ver detalhes do upload', use_container_width=True, key='history_view_v901'):
            ...
            with c2:
                if st.button('Reprocessar upload', use_container_width=True, key='history_reprocess_v901'):
                    ...
                    st.rerun()
            with c3:
                if st.button('Excluir upload específico', use_container_width=True, key='history_delete_v901'):
    ...
    with tabs[4]:
        st.subheader('Sobre')
        st.write('Versão V9.0.1: correção do upload de arquivo, mantendo calendário, YTD e o gráfico mensal da aba Tendência em barras, no mesmo estilo do semanal.')
