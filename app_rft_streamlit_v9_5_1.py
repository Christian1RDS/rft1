...
st.set_page_config(page_title='RFT Automatico - V9.5.1', page_icon='R', layout='wide', initial_sidebar_state='expanded')
...
def normalize_columns(df):
    out = df.copy(); out.columns = [str(x).strip().replace('\ufeff', '') for x in out.columns]; return out
...
def resolve_period_selection(df, ano, mode):
    ...
    if mode=='Diario':
        ...
        selected=st.sidebar.date_input('Dia', value=default_day, min_value=min_date, max_value=max_date, format='DD/MM/YYYY', key='day_v951')
        ...
        label=st.sidebar.selectbox('Semana do ano', labels, index=idx, key='week_v951')
        ...
        label=st.sidebar.selectbox('Mes do ano', labels, index=idx, key='month_v951')
...
def render_meta_resultado(metrics, meta):
    ...
    css=status_css(current_pct, meta)
    val='Sem dados' if current_pct is None else f"{current_pct:.2f}".replace('.', ',') + '%'
...
def main():
    ...
    st.markdown('<div class="hero"><div style="font-size:1.75rem;font-weight:900;">RFT Automatico - V9.5.1</div><div class="muted">Ajuste do cálculo anual: ano fecha a partir de 10/12 e termina no último dia trabalhado de dezembro com dados.</div></div>', unsafe_allow_html=True)
    with st.sidebar:
    ...
    with tabs[0]:
        if ano is None or df.empty:
            ...
            if not valid:
                ...
                with left:
                    render_resumo(info, posto, ano, min_date, max_date, selected_label, meta)
                with right:
                ...
                if hist_day.empty:
                    ...
                    st.line_chart(hist_day.set_index('Dia'), use_container_width=True)
                st.markdown('</div>', unsafe_allow_html=True)
    with tabs[1]:
        if ano is None or df.empty:
    ...
    with tabs[2]:
        ...
        import_mode=st.radio('Modo de importação', ['Somar ao historico','Substituir periodo sobreposto','Reprocessar o ano inteiro'], key='import_mode_v951')
        uploaded=st.file_uploader('Base operacional atual (.xlsx, .xls ou .csv)', type=['xlsx','xls','csv'], key='file_uploader_v951')
        prepared=None
        ...
        if st.button('Salvar arquivo localmente', type='primary', use_container_width=True, key='save_upload_v951'):
            if uploaded is None:
    ...
    with tabs[3]:
        ...
        if hist.empty:
            ...
            selected_id=st.selectbox('Selecionar upload', hist['id'].tolist(), format_func=lambda x: f'Upload {x}', key='history_select_v951')
            c1,c2,c3=st.columns(3)
            with c1:
                if st.button('Ver detalhes do upload', use_container_width=True, key='history_view_v951'):
            ...
            with c2:
                if st.button('Reprocessar upload', use_container_width=True, key='history_reprocess_v951'):
                    ...
                    st.rerun()
            with c3:
                if st.button('Excluir upload específico', use_container_width=True, key='history_delete_v951'):
                    delete_upload(conn, selected_id)
                    st.success('Upload excluído com sucesso.')
                    st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)
    with tabs[4]:
        st.markdown('<div class="panel"><div style="font-size:1.08rem;font-weight:800;">Sobre</div><div class="muted">Versão V9.5.1 com design mais bonito e corporativo. Ajuste do anual: o ano é considerado encerrado se houver dados de dezembro a partir do dia 10, e o cálculo anual finaliza no último dia trabalhado disponível desse mês. Regra visual: abaixo da meta = vermelho | acima ou igual à meta = verde.</div></div>', unsafe_allow_html=True)
